...
export async function runDailyDigest(env, options = {}) {
  ...
  if (isEmptyDigest(events)) {
    console.log('Digest: no interesting events, storing quiet entry');
    const quietMd = formatQuietDigest(since);
    await storeDigest(env, since, quietMd, events);
    await setLastSent(env);
    return { sent: false, markdown: quietMd, events: summarizeEvents(events), reason: 'nothing_interesting' };
  }
...
function formatQuietDigest(since) {
  ...
  return [
    '# Arizona Legislature Daily Digest',
    `**${today}** | Changes since ${sinceFormatted} | Source: [cactus.watch](https://cactus.watch)`,
    '',
    '## No Actions Found',
    '',
    'No floor votes, governor actions, or other tracked activity since the last digest. The legislature was likely out of session or in a quiet stretch.',
    '',
    '*Generated by [Cactus Watch](https://cactus.watch)*',
  ].join('\n');
}
